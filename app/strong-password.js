...
function chkPass(pwd) {
	...
	/* Modify overall score value based on usage vs requirements */
		...
		for (var c=0; c < arrCharsLen; c++) {
			...
			if (arrChars[c] == parseInt(minVal + 1)) { nReqChar++; oImg.className = "success"; oBonus.parentNode.className = "success"; }
			else if (arrChars[c] > parseInt(minVal + 1)) { nReqChar++; oImg.className = "exceed"; oBonus.parentNode.className = "exceed"; }
			else { oImg.className = "danger"; oBonus.parentNode.className = "danger"; }
		}
		...
		for (var c=0; c < arrCharsLen; c++) {
			...
			if (arrChars[c] == parseInt(minVal + 1)) { oImg.className = "success"; oBonus.parentNode.className = "success"; }
		...
		for (var c=0; c < arrCharsLen; c++) {
			...
			var oBonus = $(arrCharsIds[c] + 'Bonus');
			$(arrCharsIds[c]).innerHTML = arrChars[c];
			if (arrChars[c] > 0) { oImg.className = "warning"; oBonus.parentNode.className = "warning"; }
			else { 
		...
		else if (nScore >= 40 && nScore < 60) { sComplexity = "Good"; level = "progress-bar-warning"; }
		else if (nScore >= 60 && nScore < 80) { sComplexity = "Strong"; level = "progress-bar-success"; }
		else if (nScore >= 80 && nScore <= 100) { sComplexity = "Very Strong"; level = "progress-bar-info"; }
...
function initPwdChk(restart) {
	...
	for (var i in arrPassPars) { $(arrPassPars[i]).parentNode.className = "success"; }
	for (var i in arrPassDivs) { $(arrPassDivs[i]).className = "success"; }
	for (var i in arrFailPars) { $(arrFailPars[i]).parentNode.className = "danger"; }
	for (var i in arrFailDivs) { $(arrFailDivs[i]).className = "danger"; }
	$("passwordPwd").value = "";
	$("passwordTxt").value = "";
	if (restart) {
